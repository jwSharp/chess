...
def draw_clocks():
    ##### Add rect for clocks
    rect1 = pygame.draw.rect(SCREEN, GOLD, ((20, HEIGHT / 9), (WIDTH / 5 - 40, HEIGHT / 11)), 5)
    rect2 = pygame.draw.rect(SCREEN, GOLD, ((WIDTH / 1.225, HEIGHT / 9), (WIDTH / 5 - 40, HEIGHT / 11)), 5)
...
def make_gamebox(SCREEN):
    ...
    captured_box_1 = pygame.draw.rect(SCREEN, GOLD, ((20, HEIGHT / 4.5), (WIDTH / 5 - 40, HEIGHT / 2)), 5)
    captured_box_2 = pygame.draw.rect(SCREEN, GOLD, ((WIDTH / 1.225, HEIGHT / 4.5), (WIDTH / 5 - 40, HEIGHT / 2)), 5)

    captured_count1, captured_count2 = 0, 0
    for i, piece in enumerate(chess.captured_pieces):
        image = pygame.transform.smoothscale(piece.sprite, (piece.sprite.get_width() * 2, piece.sprite.get_height() * 2))
        if piece.turn == 0:
            x = captured_box_1.center[0] - image.get_width()/2
            y = (captured_box_1.top) + captured_count1 * image.get_height()/(captured_count1 + 1)
            captured_count1 += 1
        else:
            x = captured_box_2.center[0] - image.get_width()/2
            y = (captured_box_2.top) + captured_count2 * image.get_height()/(captured_count2 + 1)
            captured_count2 += 1
...
def add_player_text(SCREEN, is2p: bool):
    ...
    p1Text = playersFont.render(text_display, True, GOLD if chess.current_turn == 0 else DARK_RED, BLACK)
    ...
    p2Text = playersFont.render(text_display, True, GOLD if chess.current_turn == 1 else DARK_RED, BLACK)
...
def on_button_press(button_rect: pygame.Rect, button_str: str, font, color: tuple, hover_color: tuple): #call_func: callable, *args):
